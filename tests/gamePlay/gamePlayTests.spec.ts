...
test.describe("GamePlay Test Scenarios", async () => {
  ...
  test("Verify the generation of a random name and continue with the test account name", async ({ playPage, gamePage }) => {
  ...
  test("Verify that the Game starts in 20 seconds", async ({ playPage, gamePage }) => {
    await setupGamePlay({ playPage, gamePage })
  ...
  test.skip("Verify that the account is deleted before exiting the game", async ({ playPage, gamePage }) => {
    await setupGamePlay({ playPage, gamePage })
