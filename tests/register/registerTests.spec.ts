...
test.describe("Register Tests", async () => {
  ...
  test.skip("Register with existing email", async ({ registerPage }) => {
    ...
    expect(await registerPage.getErrorMessage()).toBe(
      registerPageErrorMessages.EXISTING_MAIL
    );
  });

  test.skip("Login With Google Account", async ({
    registerPage,
    loginPage,
  }) => {
    await registerPage.clickGoogleRegisterButton();
    await registerPage.fillEmailAtGoogleRegisterPage(
  ...
  test.skip("Open Facebook Register Page", async ({
    registerPage,
    basePage,
  }) => {
