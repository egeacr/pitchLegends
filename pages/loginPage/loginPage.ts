...
export class LoginPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.invalidUserNameOrPasswordAlert = page.locator(
      "//*[@role='alert']//span"
    );
  }
  ...
  async assertAllWarningMessages(
    ...
    try {
      ...
      expect(
        validationMessage.includes(warningMessageEN) ||
          validationMessage.includes(warningMessageTR) ||
          validationMessage.includes(emailWarning) ||
          validationMessage.includes(emailWarning_)
      );
    } catch (error) {
      console.error(
        "An error occurred while asserting warning messages:",
        error
      );
      throw error;
    }
